class Boid {
  ...
  Boid(float x, float y) {
    ...
    corner = new PVector(0, 0);
    PVector farCorner = new PVector(width, height);
    superForceCalculator = 1;
    maxDist = PVector.dist(corner, farCorner);
  }
  
  void run(ArrayList<Boid> boids) {
    dt = timer.getdt() * 100;
    timer.update();
    boolean newCornerTime = timer.checkIsCornerTime();
    superForceCalculator++;
    isCornerTime = newCornerTime;
    calculate(boids);
    flock();
    borders();
    update();
  ...
  void update() {
    // Update velocity
    // kerro acceleration ja velocity dt:llä
    velocity.add(acceleration);
    // Limit speed
    velocity.limit(maxspeed);
    //velocity = velocity.mult(dt);
